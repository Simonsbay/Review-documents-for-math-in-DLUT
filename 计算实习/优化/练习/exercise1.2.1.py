...
def fun(x):
    ...
    g = np.array([4 * (x[0] - 2) ** 3 + 2 * (x[0] - 2 * x[1]), -4 * (x[0] - 2 * x[1])])
    G = np.array([[12 * (x[0] - 2) ** 2 + 2, -4], [-4, 8]])
    return f, g, G
def f(x):
...
def nesterov(x0, eps, alpha=0.01, beta=0.9, max_iter=10000):
    xk = x0
    pk = np.zeros_like(xk)
    res = np.inf
    iter = 0
    while res > eps and iter < max_iter:
        iter += 1
        _, gk, _ = fun(xk)
        pk = beta * pk - alpha * gk
        xk_pred = xk + pk
        _, gk_pred, _ = fun(xk_pred)
        xk = xk_pred - alpha * gk_pred
        res = np.linalg.norm(gk_pred)
        print('{}th iteration, the residual is {}'.format(iter, res))
